...
def get_player_name(team, name, metadata):
    ...
    if match:
        return match
    # Try fuzzy match on just first name (e.g., "Hetymer" → "Shimron Hetmyer")
...
def main():
    ...
    global away_squad
    global metadata
    raw_files = list_json("ball-by-ball",RAW_PREFIX)
    result = []
    # for file in raw_files:
    for file in reversed(raw_files):
    # if file not in bronze_files or bronze_files == None:
